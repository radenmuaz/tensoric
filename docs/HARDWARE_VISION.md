...
*   **IC Program (Add):** Adding two numbers $A$ and $B$ where we only have their logs $X = \log_2(A)$ and $Y = \log_2(B)$ is famously difficult. Mathematically, $Z = X + \log_2(1 + 2^{Y-X})$. How do we do this purely structurally?
    *   **Strategy 1: The Structural $Max()$ Race.** In LNS, if $X$ is much larger than $Y$, then $Z \approx X$. In a Unary Wire formatting, you literally physically race an active `MAX` node down Wire $X$ and Wire $Y$ simultaneously. Whichever wire runs out first is the minimum. The `MAX` node severs the minimum wire, entirely destroying it (because $2^X + 2^{X-100} \approx 2^X$), and keeps the maximum wire $X$. **Complexity:** Because the signal travels 1 node per IC tick, the race finishes the exact moment the shorter wire ends. **Latency = $O(\min(X, Y))$ ticks.**
